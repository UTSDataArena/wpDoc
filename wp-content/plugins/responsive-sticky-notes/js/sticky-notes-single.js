...
responsivestickynotes_object.prototype = {
	...
	updateElementChain: function(id, elementChain) {
		var data = {
			...
			jQuery.ajax({
				...
				data: data,
				error: function() {
					console.log("error updating elementchain (1)")
				...
				success: function(data) {
					if (data=="error") {
						console.log("error updating elementchain: "+ note.numeridId)
					}
				},
				method: 'POST'
			});
	...
	(function($) {
		...
		function addNoteEvent(event) {
			...
			$.ajax({
				...
				error: function(e) {
				//	error - remove note
					responsivestickynotes_obj.removeNote(nextId);
